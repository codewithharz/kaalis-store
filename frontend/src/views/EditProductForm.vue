<template>
    <form @submit.prevent="handleSubmit" class="max-w-lg mx-auto mt-8">
        <div class="mb-4">
            <label for="name" class="block text-gray-700 font-bold mb-2">{{ t('editProductForm.name') }}</label>
            <input type="text" id="name" v-model="form.name" required class="w-full px-3 py-2 border rounded-lg">
        </div>
        <div class="mb-4">
            <label for="description" class="block text-gray-700 font-bold mb-2">{{ t('editProductForm.description') }}</label>
            <textarea id="description" v-model="form.description" required
                class="w-full px-3 py-2 border rounded-lg"></textarea>
        </div>
        <div class="mb-4">
            <label for="price" class="block text-gray-700 font-bold mb-2">{{ t('editProductForm.price') }}</label>
            <input type="number" id="price" v-model="form.price" required class="w-full px-3 py-2 border rounded-lg">
        </div>
        <div class="mb-4">
            <label for="stock" class="block text-gray-700 font-bold mb-2">{{ t('editProductForm.stock') }}</label>
            <input type="number" id="stock" v-model="form.stock" required class="w-full px-3 py-2 border rounded-lg">
        </div>
        <div class="mb-4">
            <label for="discount" class="block text-gray-700 font-bold mb-2">{{ t('editProductForm.discount') }}</label>
            <input type="number" id="discount" v-model="form.discount" class="w-full px-3 py-2 border rounded-lg">
        </div>
        <div class="mb-4">
            <label for="images" class="block text-gray-700 font-bold mb-2">{{ t('editProductForm.images') }}</label>
            <input type="file" id="images" @change="handleFileUpload" multiple accept="image/*"
                class="w-full px-3 py-2 border rounded-lg">
        </div>
        <div class="mb-4">
            <label for="tags" class="block text-gray-700 font-bold mb-2">{{ t('editProductForm.tags') }}</label>
            <input type="text" id="tags" v-model="form.tags" class="w-full px-3 py-2 border rounded-lg">
        </div>
        <button type="submit" class="bg-blue-500 text-white px-4 py-2 rounded-lg hover:bg-blue-600">
            {{ isEditing ? t('editProductForm.updateProduct') : t('editProductForm.createProduct') }}
        </button>
    </form>
</template>
...
export default {
    ...
    setup() {
        ...
        const handleSubmit = async (event) => {
            try {
                toast.loading(t('editProductForm.toasts.saving'));
                ...
                if (isEditing.value) {
                    ...
                    toast.success(t('editProductForm.toasts.updated'));
                } else {
                    result = await productStore.createProduct(productData);
                    toast.success(t('editProductForm.toasts.created'));
                }
                ...
                toast.error(t('editProductForm.toasts.submitFailed', { message: error.message }));
            }
        };
