    ...
    <div class="bg-gray-100 min-h-screen">
        <div class="container mx-auto px-2 sm:px-4 py-4 sm:py-8">
            ...
            <section class="mb-8 sm:mb-12">
                <h2 class="text-xl sm:text-2xl font-semibold mb-4 sm:mb-6 px-1">{{ t('sellerHomePage.featuredProducts') }}</h2>
            ...
            <section class="bg-white rounded-lg shadow p-4 sm:p-6 mb-8 sm:mb-12">
                <h2 class="text-xl sm:text-2xl font-semibold mb-3 sm:mb-4">{{ t('sellerHomePage.aboutSeller', { name: sellerProfile.storeName }) }}</h2>
                ...
                <div class="grid grid-cols-3 gap-2 sm:gap-4">
                    <div class="text-center p-2 sm:p-4 bg-gray-50 rounded-lg">
                        <p class="font-semibold text-base sm:text-lg text-gray-800">{{ sellerProfile.products?.length ||
                            0 }}</p>
                        <p class="text-gray-600 text-xs sm:text-sm">{{ t('sellerHomePage.products') }}</p>
                    </div>
                    <div class="text-center p-2 sm:p-4 bg-gray-50 rounded-lg">
                        <p class="font-semibold text-base sm:text-lg text-gray-800">{{ sellerProfile.totalSales || 0 }}
                        </p>
                        <p class="text-gray-600 text-xs sm:text-sm">{{ t('sellerHomePage.sales') }}</p>
                    </div>
                    <div class="text-center p-2 sm:p-4 bg-gray-50 rounded-lg">
                        <p class="font-semibold text-base sm:text-lg text-gray-800">{{ joinDate }}</p>
                        <p class="text-gray-600 text-xs sm:text-sm">{{ t('sellerHomePage.joined') }}</p>
                    </div>
                </div>
            </section>
            ...
            <!-- All Products -->
            <section>
                <h2 class="text-xl sm:text-2xl font-semibold mb-4 sm:mb-6 px-1">{{ t('sellerHomePage.allProducts') }}</h2>
                <div class="flex justify-center">
                    <div class="w-full max-w-7xl">
                        ...
                        <div v-if="allProducts.length === 0" class="text-center py-8 sm:py-12">
                            ...
                            <h3 class="text-lg sm:text-xl font-semibold text-gray-600 mb-2">{{ t('sellerHomePage.noProductsYet') }}</h3>
                            <p class="text-sm sm:text-base text-gray-500">{{ t('sellerHomePage.noProductsDescription') }}</p>
                        </div>
                    </div>
                </div>
            </section>
    ...
    setup() {
        ...
        const joinDate = computed(() => {
            if (sellerProfile.value && sellerProfile.value.createdAt) {
                ...
                return new Date(sellerProfile.value.createdAt).toLocaleDateString(activeLocale, { year: 'numeric', month: 'long' });
            }
            return t('sellerHomePage.notAvailable');
        });
